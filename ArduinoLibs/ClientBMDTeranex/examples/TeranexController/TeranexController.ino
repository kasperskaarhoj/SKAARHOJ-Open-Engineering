 ...
 * Allows you to control a Teranex monitor with commands over the serial monitor
 *
 * - kasper
 */
/*****************
...
void setup() {
  ...
  Teranex.begin(IPAddress(192, 168, 10, 220));	 // <= SETUP (the IP address of the Teranex)
  Teranex.serialOutput(1);
  Teranex.connect();

  while(!Teranex.hasInitialized())  {
    Teranex.runLoop();
  }


  Serial << F("Ready to control Teranex. Type any of these commands + Enter in the serial monitor:\n");
    ...
    if (loadSerialCommand())  {
      ...
      if (!strcmp_P(serialBuffer,PSTR("list")))  {
        ...
        for(uint8_t i=0; i<2; i++)  {
          ...
          Serial << F("Brightness = ") << Teranex.getBrightness(i) << F("\n");
          Serial << F("Contrast = ") << Teranex.getContrast(i) << F("\n");
          Serial << F("Saturation = ") << Teranex.getSaturation(i) << F("\n");
          Serial << F("Border = ") << Teranex.getBorder(i) << F("\n");
          Serial << F("\n");
        }
