 ...
 */
SkaarhojTCPClient::SkaarhojTCPClient() {
	_EOLChar = 10;
}
 ...
 */
void SkaarhojTCPClient::begin(IPAddress ip) {
	begin(ip, 23);
}
...
void SkaarhojTCPClient::connect() {
  ...
  if (_serialOutput) Serial.print(F("Connecting... "));
...
void SkaarhojTCPClient::disconnect() {
	if (_serialOutput) Serial.println(F("Disconnecting..."));
...
void SkaarhojTCPClient::stopClient() {
	if (_serialOutput) Serial.println(F("Stopping client."));
 ...
 */
void SkaarhojTCPClient::runLoop() {
	runLoop(0);
}
void SkaarhojTCPClient::runLoop(uint16_t delayTime) {
	...
	do {
		...
		if (!_client.connected()) {
			if (_isConnected)	{
				if (_serialOutput)	Serial.println(F("ERROR: Client not connected anymore."));
				stopClient();
			} else {
