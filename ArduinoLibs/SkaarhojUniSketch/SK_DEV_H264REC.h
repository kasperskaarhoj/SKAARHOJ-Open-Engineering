...
uint16_t evaluateAction_H264REC(const uint8_t devIndex, const uint16_t actionPtr, const uint8_t HWc, const uint8_t actIdx, bool actDown, bool actUp, int16_t pulses, int16_t value) {
  uint16_t retVal = 0;
  int16_t tempInt = 0;
  uint8_t tempByte = 0;
  ...
  if (actDown || actUp) {
    if (debugMode)
      Serial << F("H264 Recorder action ") << globalConfigMem[actionPtr] << F("\n");
  }
