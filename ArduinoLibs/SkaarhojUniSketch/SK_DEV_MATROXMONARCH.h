...
uint16_t evaluateAction_MATROXMONARCH(const uint8_t devIndex, const uint16_t actionPtr, const uint8_t HWc, const uint8_t actIdx, bool actDown, bool actUp, int16_t pulses, int16_t value) {
  uint16_t retVal = 0;
  int16_t tempInt = 0;
  uint8_t tempByte = 0;
  ...
  if (actDown || actUp) {
    if (debugMode)
      Serial << F("Matrox Monarch action ") << globalConfigMem[actionPtr] << F("\n");
  }
