namespace ATEM {
  ...
  uint16_t vSrcMap[] = {1000, 2001, 2002, 3010, 3011, 3020, 3021, 6000, 7001, 7002, 10010, 10011, 10020, 10021};
  uint16_t aSrcMap[] = {1001, 1101, 1201, 2001, 2002};
  #define HOLDGROUPSIZE 5
  uint16_t holdGroup[2][HOLDGROUPSIZE];
  uint8_t holdGroupHWc[2][HOLDGROUPSIZE];
  uint8_t holdGroupPtr[2] = {0, 0};

  /**
   * Index to ATEM video source (aligned with selector box in web interface)
   */
  uint16_t idxToVideoSrc(uint8_t devIndex, uint8_t idx) {
    if (idx <= 20) {
      return idx;
    } else if (idx >= 21 && idx <= 30) {
      return vSrcMap[idx - 21];
    } else if (idx >= 31 && idx <= 36) { // AUX's
      return AtemSwitcher[devIndex].getAuxSourceInput(idx - 31);
    } else if (idx >= 37 && idx <= 40) {
      return vSrcMap[idx - 27];
      ...
      if (_systemMem[idx - 57] < 57)
        return idxToVideoSrc(devIndex, _systemMem[idx - 57]); // Recursive call, but making sure we are not asking for another memory value which would make the loop infinite
    }
    return -1;
  }
   ...
   */
  uint16_t idxToAudioSrc(uint8_t devIndex, uint8_t idx) {
    if (idx < 20) { // Regular inputs
      return idx + 1;
    } else if (idx >= 20 && idx <= 24) { // Special inputs
      return aSrcMap[idx - 20];
    } else if (idx == 25 || idx == 26) { // Master and Monitor
      return idx;
    } else if (idx >= 27 && idx <= 30) { // Memory banks
      if (_systemMem[idx - 27] < 27)
        return idxToAudioSrc(devIndex, _systemMem[idx - 27]); // Recursive call, but making sure we are not asking for another memory value which would make the loop infinite
    }
    return 0;
  }
  ...
  bool pushToHoldGroup(uint8_t hgIdx, uint16_t src, uint8_t HWc) {
    if (holdGroupPtr[hgIdx] < HOLDGROUPSIZE) {
  ...
  uint16_t pullFromHoldGroup(uint8_t hgIdx, uint8_t HWc) {
    uint8_t ai = 0;
    uint8_t origPtr = holdGroupPtr[hgIdx] < HOLDGROUPSIZE ? holdGroupPtr[hgIdx] : HOLDGROUPSIZE;
   ...
   */
  uint16_t searchVideoSrc(uint8_t devIndex, uint16_t src, int16_t pulseCount, uint8_t filter, uint8_t filterME, uint8_t limit) {
   ...
   */
  uint16_t searchMediaStill(uint8_t devIndex, uint8_t srcI, int16_t pulseCount, uint8_t limit) {
   ...
   */
  uint16_t searchMacro(uint8_t devIndex, uint8_t macroIdx, int16_t pulseCount, uint8_t limit) {
  ...
  uint16_t evaluateAction(const uint8_t devIndex, const uint16_t actionPtr, const uint8_t HWc, const uint8_t actIdx, bool actDown, bool actUp, int16_t pulses, int16_t value, uint8_t HWcType) {
    ...
    case 0: // Program Source
      ...
      retVal = globalConfigMem[actionPtr + 3] == 3 ? (_systemHWcActionCacheFlag[HWc][actIdx] ? (2 | 0x20) : 5) : (AtemSwitcher[devIndex].getProgramInputVideoSource(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? (2 | 0x20) : 5);

      // The availabilty should not affect the color in cycle mode
      if (globalConfigMem[actionPtr + 3] != 3 && !(AtemSwitcher[devIndex].getInputMEAvailability(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2])) & (B1 << globalConfigMem[actionPtr + 1]))) {
        retVal = 0;
      }
      ...
      if (extRetValIsWanted()) {
        ...
        if (_systemHWcActionPrefersLabel[HWc] && globalConfigMem[actionPtr + 3] != 3) {
          ...
          extRetValTxt(AtemSwitcher[devIndex].getInputLongName(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2])), 0);
          extRetValTxtShort(AtemSwitcher[devIndex].getInputShortName(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2])));
        } else {
    ...
    case 1: // Preview Source
      ...
      retVal = globalConfigMem[actionPtr + 3] == 1 ? (_systemHWcActionCacheFlag[HWc][actIdx] ? (3 | 0x20) : 5) : (AtemSwitcher[devIndex].getPreviewInputVideoSource(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? (3 | 0x20) : 5);

      // The availabilty should not affect the color in cycle mode
      if (globalConfigMem[actionPtr + 3] != 1 && !(AtemSwitcher[devIndex].getInputMEAvailability(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2])) & (B1 << globalConfigMem[actionPtr + 1]))) {
        retVal = 0;
      }
    ...
    case 2: // Program/Preview Source
      ...
      retVal = globalConfigMem[actionPtr + 3] == 1 ? (_systemHWcActionCacheFlag[HWc][actIdx] ? (4 | 0x20) : 5) : (AtemSwitcher[devIndex].getProgramInputVideoSource(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? 2 : (AtemSwitcher[devIndex].getPreviewInputVideoSource(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? 3 : 5));
      if (retVal != 5)
    ...
    case 3: // AUX
      ...
      retVal = globalConfigMem[actionPtr + 3] == 5 ? (_systemHWcActionCacheFlag[HWc][actIdx] ? (4 | 0x20) : 5) : (AtemSwitcher[devIndex].getAuxSourceInput(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? (4 | 0x20) : 5);

      // The availabilty should not affect the color in cycle mode
      if (globalConfigMem[actionPtr + 3] != 5 && !(AtemSwitcher[devIndex].getInputAvailability(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2])) & B1)) {
        retVal = 0;
      }
    ...
    case 5: // Upstream Keyer Fill
      ...
      if (pulses & 0xFFFE) {
        AtemSwitcher[devIndex].setKeyerFillSource(globalConfigMem[actionPtr + 1], globalConfigMem[actionPtr + 2], searchVideoSrc(devIndex, AtemSwitcher[devIndex].getKeyerFillSource(globalConfigMem[actionPtr + 1], globalConfigMem[actionPtr + 2]), (pulses >> 1), 0, B1, globalConfigMem[actionPtr + 3]));
      }

      retVal = AtemSwitcher[devIndex].getKeyerFillSource(globalConfigMem[actionPtr + 1], globalConfigMem[actionPtr + 2]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 3]) ? (4 | 0x20) : 5;
      ...
      if (extRetValIsWanted()) {
        ...
        if (_systemHWcActionPrefersLabel[HWc]) {
          extRetValColor(retVal & 0x20 ? B110111 : B101010);
          extRetValSetLabel(true);
          extRetValTxt(AtemSwitcher[devIndex].getInputLongName(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 3])), 1);
        } else {
          extRetValColor(B101010);
    ...
    case 6: // Upstream Keyer Key
      ...
      retVal = AtemSwitcher[devIndex].getKeyerKeySource(globalConfigMem[actionPtr + 1], globalConfigMem[actionPtr + 2]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 3]) ? (4 | 0x20) : 5;
    ...
    case 8: // Downstream Keyer Fill
      ...
      retVal = AtemSwitcher[devIndex].getDownstreamKeyerFillSource(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? (4 | 0x20) : 5;
      ...
      if (extRetValIsWanted()) {
        ...
        if (_systemHWcActionPrefersLabel[HWc]) {
          extRetValSetLabel(true);
          extRetValColor(retVal & 0x20 ? B011111 : B101010);
          extRetValTxt(AtemSwitcher[devIndex].getInputLongName(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2])), 1);
        } else {
          extRetValColor(B101010);
    ...
    case 9: // Downstream Keyer Key
      ...
      retVal = AtemSwitcher[devIndex].getDownstreamKeyerKeySource(globalConfigMem[actionPtr + 1]) == idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 2]) ? (4 | 0x20) : 5;
    ...
    case 10: // MP Still
      ...
      if (pulses & 0xFFFE) {
        AtemSwitcher[devIndex].setMediaPlayerSourceStillIndex(globalConfigMem[actionPtr + 1], searchMediaStill(devIndex, AtemSwitcher[devIndex].getMediaPlayerSourceStillIndex(globalConfigMem[actionPtr + 1]), (pulses >> 1), globalConfigMem[actionPtr + 2] - 1));
      }
    ...
    case 25: { // Audio Levels
      ...
      uint8_t aSrc = idxToAudioSrc(devIndex, globalConfigMem[actionPtr + 1]);
    ...
    case 43: // CCU Settings
      ...
      if (_systemHWcActionCacheFlag[HWc][actIdx] & 16) {
        ...
        if ((uint16_t)millis() - lastSettingsRecall < 10000 && lastLoadedPreset == globalConfigMem[actionPtr + 3] && lastLoadedCamera == cam) {
    ...
    case 45: // Video Tally
      ...
      case 1:
      case 2:
        retVal = AtemSwitcher[devIndex].getTallyFlags(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 1])) & B10 ? (3 | 0x20) : 5;
      ...
      case 0:
      case 2:
        retVal = AtemSwitcher[devIndex].getTallyFlags(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 1])) & B1 ? (2 | 0x20) : retVal;
        break;
      }
      ...
      if (extRetValIsWanted()) {
        ...
        extRetValSetLabel(true);
        extRetValTxt(AtemSwitcher[devIndex].getInputLongName(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 1])), 0);
        extRetValTxtShort(AtemSwitcher[devIndex].getInputShortName(idxToVideoSrc(devIndex, globalConfigMem[actionPtr + 1])));
      }
      return retVal;
      break;
    case 46: // Audio Tally
      aSrc = idxToAudioSrc(devIndex, globalConfigMem[actionPtr + 1]);
      retVal = AtemSwitcher[devIndex].getAudioTallyFlags(idxToAudioSrc(devIndex, globalConfigMem[actionPtr + 1])) ? (4 | 0x20) : 5;
