...
void webDefaultView(WebServer &server, WebServer::ConnectionType type) {
  ...
  if (type == WebServer::POST) {
    ...
    do {
	  ...
	  if (!endConfigParsing)	{
	      ...
	      if (!strncmp_P(fieldName, PSTR("selfnc_"), 7)) {
	        ...
	        if (HWCid == HWCPtr + 1)    {
	          ...
	          HWCPtr++;
	          cStateIndex = -1;
	          if (globalConfigMemPtr < SK_CONFIG_MEMORY_SIZE) globalConfigMem[globalConfigMemPtr++] = 255; // Length, to be updated
	        }
	        ...
	        if (globalConfigMemPtr < SK_CONFIG_MEMORY_SIZE) globalConfigMem[globalConfigMemPtr++] = (atoi(strtok(fieldValue, "_")) & 0xF) | (logic << 4); // Device + logic
	        if (globalConfigMemPtr < SK_CONFIG_MEMORY_SIZE) globalConfigMem[globalConfigMemPtr++] = atoi(strtok(NULL, "_"));    // Function
	        len = 2;
	      }
	      ...
	      if (!strncmp_P(fieldName, PSTR("selval_"), 7)) {  // TODO: Check it's the values matching the previous function...
	        if (len < 7 + 2)   {
	          len++;
	          if (globalConfigMemPtr < SK_CONFIG_MEMORY_SIZE) globalConfigMem[globalConfigMemPtr++] = atoi(fieldValue);
	        }
	      }
      ...
      if (!strncmp_P(fieldName, PSTR("ctrlIP_"), 7)) {
		...
		if (IPidx<4) globalConfigMem[globalConfigMemPtr+IPidx] = atoi(fieldValue);
	  }
      if (!strncmp_P(fieldName, PSTR("subnet_"), 7)) {
        IPidx = atoi(strtok(fieldName + 7, "_"));
		if (IPidx<4) globalConfigMem[globalConfigMemPtr+4+IPidx] = atoi(fieldValue);
	  }
      if (!strncmp_P(fieldName, PSTR("devIP_"), 6)) {
        g = atoi(strtok(fieldName + 6, "-"));
		IPidx = atoi(strtok(NULL, "-"));
		if (IPidx<4 && g<SK_DEVICES) globalConfigMem[globalConfigMemPtr+8+1+g*5+IPidx+1] = atoi(fieldValue);
		globalConfigMem[globalConfigMemPtr+8] = SK_DEVICES*5;
	  }
      if (!strncmp_P(fieldName, PSTR("devEn_"), 6)) {
        g = atoi(fieldName + 6);
		if (g<SK_DEVICES) globalConfigMem[globalConfigMemPtr+8+1+g*5] = 1;
	  }
  ...
  P(htmlHead) = 
    "<html><head><title>SKAARHOJ Controller</title>"
    "<style id=\"st\"></style>"
  ...
  uint8_t nStates = globalConfigMem[ptr];
//  Serial << nStates << "\n";
//  Serial << ptr << "\n";
//  Serial << globalConfigMem[ptr] << "\n";
  ...
  server << F("<input type=\"submit\" value=\"Save Settings\" class=\"sBut\"></form></body></html>");
}
