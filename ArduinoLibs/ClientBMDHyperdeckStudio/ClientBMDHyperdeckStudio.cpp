...
void ClientBMDHyperdeckStudio::disconnect() {
	if (_serialOutput) Serial.println(F("Disconnecting from Hyperdeck Studio..."));
...
void ClientBMDHyperdeckStudio::_parseline()	{
	...
	if (!strcmp(_buffer,""))	{
		...
		if (_serialOutput)	Serial.println(F("Connection confirmed, pulling status"));
		...
		if (_serialOutput)	Serial.println(F("Rejected."));
		_wasRejected = true;
		...
		switch(_section)	{
			case 202: 	// Slot info:
				if (isNextPartOfBuffer_P(PSTR("slot id: ")))	{
					...
					if (_Hyperdeck_currentSlotId<1 || _Hyperdeck_currentSlotId>2)	{_Hyperdeck_currentSlotId=1;}	// Safety measure!
					
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_currentSlotId="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_currentSlotId);
				}
					...
					if (isNextPartOfBuffer_P(PSTR("mounted")))	{	// mounted==1, 
						...
						if (_serialOutput>1) Serial.print(F("UNKNOWN status:"));
						if (_serialOutput>1) Serial.println(_buffer+_bufferReadIndex);
						_Hyperdeck_slotStatus[_Hyperdeck_currentSlotId-1] = 0;
					}
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_slotStatus="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_slotStatus[_Hyperdeck_currentSlotId-1]);
				}

				else if (isNextPartOfBuffer_P(PSTR("recording time: ")))	{
					_Hyperdeck_slotRecordingTime[_Hyperdeck_currentSlotId-1] = parseInt();
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_slotRecordingTime="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_slotRecordingTime[_Hyperdeck_currentSlotId-1]);
				}
					...
					if (_serialOutput>1) Serial.print(F("UNKNOWN type:"));
					if (_serialOutput>1) Serial.println(_buffer+_bufferReadIndex);
				}
			break;
			case 205:
			...
			case 208: 	// Transport info:
				if (isNextPartOfBuffer_P(PSTR("status: ")))	{
					...
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_transportStatus="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_transportStatus);
				}

				else if (isNextPartOfBuffer_P(PSTR("speed: ")))	{
					_Hyperdeck_playSpeed = parseInt();
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_playSpeed="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_playSpeed);
				}

				else if (isNextPartOfBuffer_P(PSTR("slot id: ")))	{
					_Hyperdeck_playBackSlotId = parseInt();
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_playBackSlotId="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_playBackSlotId);
				}
					...
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_clipId="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_clipId);
				}
					...
					if (isNextPartOfBuffer_P(PSTR("NTSC")))	{
						...
						if (_serialOutput>1) Serial.print(F("UNKNOWN video format:"));
						if (_serialOutput>1) Serial.println(_buffer+_bufferReadIndex);
						_Hyperdeck_videoFormat = 0;	// Unknown
					}
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_videoFormat="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_videoFormat);
				}
			...
			case 209: 	// Notify:
				...
				if (_serialOutput>1) Serial.print(F("_Hyperdeck_notifyStatus="));
				if (_serialOutput>1) Serial.println(_Hyperdeck_notifyStatus);
			break;
			case 210: 	// Remote:
				if (isNextPartOfBuffer_P(PSTR("enabled: ")))	{
					_Hyperdeck_remoteEnabled = isNextPartOfBuffer_P(PSTR("true"));
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_remoteEnabled="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_remoteEnabled);
				}
				else if (isNextPartOfBuffer_P(PSTR("override: ")))	{
					_Hyperdeck_remoteOverride = isNextPartOfBuffer_P(PSTR("true"));
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_remoteOverride="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_remoteOverride);
				}
			...
			case 211: 	// Configuration:
				if (isNextPartOfBuffer_P(PSTR("audio input: ")))	{
					if (isNextPartOfBuffer_P(PSTR("embedded")))	{	// embedded==1
						...
						if (_serialOutput>1) Serial.print(F("UNKNOWN audio format:"));
						if (_serialOutput>1) Serial.println(_buffer+_bufferReadIndex);
						_Hyperdeck_audioInput = 0;	// Unknown
					}
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_audioInput="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_audioInput);
				}
					...
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_videoInput="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_videoInput);
				}
					...
					if (isNextPartOfBuffer_P(PSTR("QuickTimeUncompressed")))	{
						...
						if (_serialOutput>1) Serial.print(F("UNKNOWN file format:"));
						if (_serialOutput>1) Serial.println(_buffer+_bufferReadIndex);
						_Hyperdeck_fileFormat = 0;	// Unknown
					}
					if (_serialOutput>1) Serial.print(F("_Hyperdeck_fileFormat="));
					if (_serialOutput>1) Serial.println(_Hyperdeck_fileFormat);
				}
...
uint8_t ClientBMDHyperdeckStudio::getCurrentSlotId()	{
	return _Hyperdeck_currentSlotId;
}
uint8_t ClientBMDHyperdeckStudio::getSlotStatus(uint8_t slotId)	{
	return _Hyperdeck_slotStatus[slotId-1];
}
unsigned long ClientBMDHyperdeckStudio::getSlotRecordingTime(uint8_t slotId) {
	return _Hyperdeck_slotRecordingTime[slotId-1];
}
uint8_t ClientBMDHyperdeckStudio::getTransportStatus() {
	return _Hyperdeck_transportStatus;
}
bool ClientBMDHyperdeckStudio::isPlaying() {
	return _Hyperdeck_transportStatus == ClientBMDHyperdeckStudio_TRANSPORT_PLAY;
}
bool ClientBMDHyperdeckStudio::isRecording() {
	return _Hyperdeck_transportStatus == ClientBMDHyperdeckStudio_TRANSPORT_RECORD;
}
bool ClientBMDHyperdeckStudio::isStopped() {
	return _Hyperdeck_transportStatus == ClientBMDHyperdeckStudio_TRANSPORT_STOPPED;
}
bool ClientBMDHyperdeckStudio::isInPreview() {
	return _Hyperdeck_transportStatus == ClientBMDHyperdeckStudio_TRANSPORT_PREVIEW;
}
int ClientBMDHyperdeckStudio::getPlaySpeed() {
	return _Hyperdeck_playSpeed;
}
uint8_t ClientBMDHyperdeckStudio::getClipId() {
	return _Hyperdeck_clipId;
}
uint8_t ClientBMDHyperdeckStudio::getPlayBackSlotId() {
	return _Hyperdeck_playBackSlotId;
}
uint8_t ClientBMDHyperdeckStudio::getTChh() {
	return _Hyperdeck_tc_hh;
}
uint8_t ClientBMDHyperdeckStudio::getTCmm()	{
	return _Hyperdeck_tc_mm;
}
uint8_t ClientBMDHyperdeckStudio::getTCss() {
	return _Hyperdeck_tc_ss;
}
uint8_t ClientBMDHyperdeckStudio::getTCff() {
	return _Hyperdeck_tc_ff;
}
uint8_t ClientBMDHyperdeckStudio::getVideoFormat() {
	return _Hyperdeck_videoFormat;
}
bool ClientBMDHyperdeckStudio::isRemoteEnabled() {
	return _Hyperdeck_remoteEnabled;
}
bool ClientBMDHyperdeckStudio::isRemoteOverride() {
	return _Hyperdeck_remoteOverride;
}
uint8_t ClientBMDHyperdeckStudio::getAudioInput() {
	return _Hyperdeck_audioInput;
}
uint8_t ClientBMDHyperdeckStudio::getVideoInput() {
	return _Hyperdeck_videoInput;
}
uint8_t ClientBMDHyperdeckStudio::getFileFormat() {
	return _Hyperdeck_fileFormat;
}
int ClientBMDHyperdeckStudio::getInputStatus() {
	return _noInput;
}
uint8_t ClientBMDHyperdeckStudio::getTotalClipCount() {
	return _Hyperdeck_clipCount;
}
uint8_t ClientBMDHyperdeckStudio::getFileClipId(uint8_t index) {
	return _Hyperdeck_fileidlist[index];
}
...
void ClientBMDHyperdeckStudio::gotoTimecode(uint8_t hh, uint8_t mm, uint8_t ss, uint8_t ff)	{
	gotoTimecode(hh, mm, ss, ff, 0);
}
...
void ClientBMDHyperdeckStudio::goForwardInTimecode(uint8_t hh, uint8_t mm, uint8_t ss, uint8_t ff) {
	gotoTimecode(hh, mm, ss, ff, 1);
}
    
// go backward {timecode} duration
void ClientBMDHyperdeckStudio::goBackwardsInTimecode(uint8_t hh, uint8_t mm, uint8_t ss, uint8_t ff) {
	gotoTimecode(hh, mm, ss, ff, -1);
}
