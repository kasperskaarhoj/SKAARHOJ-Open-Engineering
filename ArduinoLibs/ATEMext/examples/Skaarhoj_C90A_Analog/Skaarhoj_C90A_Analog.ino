...
SkaarhojUtils utils1;
SkaarhojUtils utils2;
SkaarhojUtils utils3;
SkaarhojUtils utils4;
SkaarhojUtils utils5;
SkaarhojUtils utils6;
SkaarhojUtils utils7;

#include <SkaarhojAudioControl.h>
SkaarhojAudioControl AudioControl1;
SkaarhojAudioControl AudioControl2;
SkaarhojAudioControl AudioControl3;
SkaarhojAudioControl AudioControl4;
...
void webDefaultView(WebServer &server, WebServer::ConnectionType type)
{
  ...
  for (i = 1; i <= 8; ++i)
  {
    ...
    server << F("<option value='") << 26 << F("'") << (EEPROM.read(400 + i) == 26 ? F(" selected='selected'") : F("")) << F(">Media2") << F("</option>");
    server << F("<option value='") << 27 << F("'") << (EEPROM.read(400 + i) == 27 ? F(" selected='selected'") : F("")) << F(">SuperSource") << F("</option>");

    server << F("<option value='") << 255 << F("'") << (EEPROM.read(400 + i) == 255 ? F(" selected='selected'") : F("")) << F(">Multiviewer") << F("</option>");
  ...
  for (i = 1; i <= 2; ++i)
  {
    ...
    server << F("<option value='") << 0 << F("'") << (EEPROM.read(190 + i) == 0 ? F(" selected='selected'") : F("")) << F("> \t") << F("</option>");

    // Audio
    server << F("<option value='") << 1 << F("'") << (EEPROM.read(190 + i) == 1 ? F(" selected='selected'") : F("")) << F(">Mute All") << F("</option>");
    server << F("<option value='") << 2 << F("'") << (EEPROM.read(190 + i) == 2 ? F(" selected='selected'") : F("")) << F(">AFV All") << F("</option>");
    server << F("<option value='") << 3 << F("'") << (EEPROM.read(190 + i) == 3 ? F(" selected='selected'") : F("")) << F(">On All") << F("</option>");
    server << F("<option value='") << 4 << F("'") << (EEPROM.read(190 + i) == 4 ? F(" selected='selected'") : F("")) << F(">MPL1 AFV") << F("</option>");
    server << F("<option value='") << 5 << F("'") << (EEPROM.read(190 + i) == 5 ? F(" selected='selected'") : F("")) << F(">MPL2 AFV") << F("</option>");

    // FTB
    server << F("<option value='") << 6 << F("'") << (EEPROM.read(190 + i) == 6 ? F(" selected='selected'") : F("")) << F(">FTB") << F("</option>");

    // USK 1-4 on/off
    server << F("<option value='") << 7 << F("'") << (EEPROM.read(190 + i) == 7 ? F(" selected='selected'") : F("")) << F(">USK1") << F("</option>");
    server << F("<option value='") << 8 << F("'") << (EEPROM.read(190 + i) == 8 ? F(" selected='selected'") : F("")) << F(">USK2") << F("</option>");
    server << F("<option value='") << 9 << F("'") << (EEPROM.read(190 + i) == 9 ? F(" selected='selected'") : F("")) << F(">USK3") << F("</option>");
    server << F("<option value='") << 10 << F("'") << (EEPROM.read(190 + i) == 10 ? F(" selected='selected'") : F("")) << F(">USK4") << F("</option>");

    // DSK 1-2 on/off
    server << F("<option value='") << 11 << F("'") << (EEPROM.read(190 + i) == 11 ? F(" selected='selected'") : F("")) << F(">DSK1") << F("</option>");
    server << F("<option value='") << 12 << F("'") << (EEPROM.read(190 + i) == 12 ? F(" selected='selected'") : F("")) << F(">DSK2") << F("</option>");
...
void setup() {
  ...
  if (isConfigMode)  {
    ...
    utils1.uniDirectionalSlider_init(10, 0, 0, A12);
    utils1.uniDirectionalSlider_disableUnidirectionality(true);
    utils2.uniDirectionalSlider_init(10, 0, 0, A13);
    utils2.uniDirectionalSlider_disableUnidirectionality(true);

    utils3.uniDirectionalSlider_init(10, 0, 0, A14);
    utils3.uniDirectionalSlider_disableUnidirectionality(true);
    utils4.uniDirectionalSlider_init(10, 0, 0, A15);
    utils4.uniDirectionalSlider_disableUnidirectionality(true);

    utils5.uniDirectionalSlider_init(10, 0, 0, A4);
    utils5.uniDirectionalSlider_disableUnidirectionality(true);
    utils6.uniDirectionalSlider_init(10, 0, 0, A5);
    utils6.uniDirectionalSlider_disableUnidirectionality(true);

    utils7.uniDirectionalSlider_init(10, 0, 0, A2);
    utils7.uniDirectionalSlider_disableUnidirectionality(true);

    AudioControl1.begin(2);	// Address 2
    AudioControl2.begin(3);	// Address 3
    AudioControl3.begin(4);	// Address 4
    AudioControl4.begin(5);	// Address 5
...
uint8_t potCalibrationLimits_len = 7;
int potCalibrationLimits_potValue[7] = {
  0, 150, 491, 647, 754, 872, 1000
};
int potCalibrationLimits_dbValue[7] = {
  -60, -20, -9, -6, -3, 0, 6
};  // -60 = 0, +6 = 1000
...
void AudioControl()  {
  ...
  if (utils1.uniDirectionalSlider_hasMoved() )  {
    ...
    AtemSwitcher.setAudioMixerInputVolume(theChannel, AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils1.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
  ...
  if (utils2.uniDirectionalSlider_hasMoved()  )  {
    ...
    AtemSwitcher.setAudioMixerInputVolume(theChannel, AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils2.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
  ...
  if (utils3.uniDirectionalSlider_hasMoved()  )  {
    ...
    AtemSwitcher.setAudioMixerInputVolume(theChannel, AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils3.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
  ...
  if (utils4.uniDirectionalSlider_hasMoved()  )  {
    ...
    AtemSwitcher.setAudioMixerInputVolume(theChannel, AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils4.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
  ...
  if (utils5.uniDirectionalSlider_hasMoved()  )  {
    ...
    AtemSwitcher.setAudioMixerInputVolume(theChannel, AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils5.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
  ...
  AudioControl3.setChannelIndicatorLight(2, readOutChannel == 6 ? (millis() & 1024 ? B01 : B00) : 0);
  if (utils6.uniDirectionalSlider_hasMoved()  )  {
    Serial << "Pos6: " << utils6.uniDirectionalSlider_position() << "\n";
    AtemSwitcher.setAudioMixerInputVolume(theChannel, AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils6.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
  ...
  if (utils7.uniDirectionalSlider_hasMoved()  )  {
    ...
    AtemSwitcher.setAudioMixerMasterVolume(AtemSwitcher.audioDb2Word(-60 + (float)(1000 - utils7.uniDirectionalSlider_position()) / 1000 * (60 + 6)));
  }
...
uint16_t inputReturn (uint8_t inputGet, uint8_t inputPosition) {
  ...
  if (inputGet == 0) {
    ...
    returnedValue == 600;
  } else if (inputGet == 255) {
    returnedValue = AtemSwitcher.getMultiViewerInputVideoSource(0, inputPosition + 1);
  } else {
...
void runTest() {
  ...
  lDelay(20);
  cmdSelect.testProgramme(B10110011);
  lDelay(20);
}
