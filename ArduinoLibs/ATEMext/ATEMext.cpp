...
void ATEMext::_parseGetCommands(const char *cmdStr)	{
	...
	if(!strcmp_P(cmdStr, PSTR("InPr"))) {
		...
		if (getVideoSrcIndex(videoSource)<=46) {
			...
			#endif					
			atemInputAvailability[getVideoSrcIndex(videoSource)] = _packetBuffer[32];
			#if ATEM_debug
			...
			#endif					
			atemInputMEAvailability[getVideoSrcIndex(videoSource)] = _packetBuffer[33];
			#if ATEM_debug
	...
	void ATEMext::setCameraControlFocus(uint8_t input, int focus) {
		...
		_packetBuffer[12+_cBBO+4+4+3] = 0x01;	// ?			
		_packetBuffer[12+_cBBO+4+4+4] = 0x80;	// ?
		_packetBuffer[12+_cBBO+4+4+9] = 0x01;	// One byte		
