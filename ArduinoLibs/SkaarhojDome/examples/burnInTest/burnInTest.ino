/*****************
 * This example will run the LED test sequence on 4 SkaarhojDome boards on the I2C bus (if they exist)
...
// All related to library "SkaarhojDome":
#include "Wire.h"
#include "MCP23017.h"
#include "PCA9685.h"
...
SkaarhojDome boardArray[] = {
  SkaarhojDome(), SkaarhojDome(), SkaarhojDome(), SkaarhojDome(), SkaarhojDome(), SkaarhojDome(), SkaarhojDome(), SkaarhojDome()};
bool boardArrayEnabled[8];



void setup() { 
  Serial.begin(11500); // set up serial
  Serial << F("\n- - - - - - - -\nSerial Started\n");

  // Shows free memory:  
  //Serial << F("freeMemory()=") << freeMemory() << "\n";

  // Always initialize Wire before setting up the SkaarhojDome class!
  Wire.begin();
 ...
 */
void buttonPressSession(uint8_t bDown, SkaarhojDome &board)  {
  if (bDown)  {
