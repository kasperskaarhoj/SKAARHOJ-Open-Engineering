...
uint8_t readOutChannel = 0;
uint8_t readOutChannelMap[7] = {0, 4,5,6,7,2,8};

  // Calibration for the particular markings of the knobs:
uint8_t potCalibrationLimits_len = 7;
int potCalibrationLimits_potValue[7] = {0, 150, 491, 647, 754, 872, 1000};
int potCalibrationLimits_dbValue[7] = {-60, -20, -9, -6, -3, 0, 6};  // -60 = 0, +6 = 1000
  ...
  if (isConfigMode)  {
    ...
    if (AtemSwitcher.hasInitialized())  {
      if (!AtemOnline)  {
        ...
        AtemSwitcher.sendAudioLevelNumbers(true);
        
        // TEMPORARY workaround in order to make sure we know the channel states:
        AtemSwitcher.changeAudioChannelMode(0, 1);
        AtemSwitcher.delay(100);
        AtemSwitcher.changeAudioChannelMode(1, 1);
        AtemSwitcher.changeAudioChannelMode(2, 1);
        AtemSwitcher.delay(100);
        AtemSwitcher.changeAudioChannelMode(3, 1);
        AtemSwitcher.changeAudioChannelMode(4, 1);
        AtemSwitcher.delay(100);
        AtemSwitcher.changeAudioChannelMode(5, 1);
        AtemSwitcher.changeAudioChannelMode(6, 1);
        AtemSwitcher.delay(100);

        AtemSwitcher.changeAudioChannelMode(0, 0);
        AtemSwitcher.delay(100);
        AtemSwitcher.changeAudioChannelMode(1, 0);
        AtemSwitcher.changeAudioChannelMode(2, 0);
        AtemSwitcher.delay(100);
        AtemSwitcher.changeAudioChannelMode(3, 0);
        AtemSwitcher.changeAudioChannelMode(4, 0);
        AtemSwitcher.delay(100);
        AtemSwitcher.changeAudioChannelMode(5, 0);
        AtemSwitcher.changeAudioChannelMode(6, 0);
        AtemSwitcher.delay(100);
      }
...
//  input = 1;
//  button = input>4 ? input-4 : input+4;
  input = 3;
  button = 5;
  if (inputSelect.isButtonIn(button, busSelection))  { 
    AtemSwitcher.changePreviewInput(input); 
  }
  inputSelect.setButtonColor(button, AtemSwitcher.getProgramTally(input) ? 2 : (AtemSwitcher.getPreviewTally(input) ? 3 : 5));
...
//  button = input>4 ? input-4 : input+4;
  ...
  inputSelect.setButtonColor(button, AtemSwitcher.getProgramInput()==input ? 2 : (AtemSwitcher.getPreviewInput()==input ? 3 : 5));
...
void AudioControl()  {
  ...
  theChannel = 2;
  if(buttons & (1 << 0)) {
    AtemSwitcher.changeAudioChannelMode(theChannel, AtemSwitcher.getAudioChannelMode(theChannel)==1?0:1);
  }
  AudioControl1.setButtonLight(1, AtemSwitcher.getAudioChannelMode(theChannel)==1);
  if(buttons & (1 << 1)) {
    AtemSwitcher.changeAudioChannelMode(theChannel, AtemSwitcher.getAudioChannelMode(theChannel)==2?0:2);
  }
  AudioControl1.setChannelIndicatorLight(1,readOutChannel==1 ? (millis()&1024?B01:B00) : 0);
  AudioControl1.setButtonLight(2, AtemSwitcher.getAudioChannelMode(theChannel)==2);
  if (utils1.uniDirectionalSlider_hasMoved() )  {
    Serial << "Pos1: " << utils1.uniDirectionalSlider_position() << "\n";
    AtemSwitcher.changeAudioChannelVolume(theChannel, calcAudioLevel(utils1.uniDirectionalSlider_position()))	;
  }
  ...
  AudioControl1.setButtonLight(3, AtemSwitcher.getAudioChannelMode(theChannel)==1);
  if(buttons & (1 << 3)) {
    AtemSwitcher.changeAudioChannelMode(theChannel, AtemSwitcher.getAudioChannelMode(theChannel)==2?0:2);
  }
  AudioControl1.setChannelIndicatorLight(2,readOutChannel==2 ? (millis()&1024?B01:B00) : 0);
  AudioControl1.setButtonLight(4, AtemSwitcher.getAudioChannelMode(theChannel)==2);
  if (utils2.uniDirectionalSlider_hasMoved()  )  {
    Serial << "Pos2: " << utils2.uniDirectionalSlider_position() << "\n";
    AtemSwitcher.changeAudioChannelVolume(theChannel, calcAudioLevel(utils2.uniDirectionalSlider_position()))	;
  }
  ...
  AudioControl2.setButtonLight(1, AtemSwitcher.getAudioChannelMode(theChannel)==1);
  if(buttons & (1 << 1)) {
    AtemSwitcher.changeAudioChannelMode(theChannel, AtemSwitcher.getAudioChannelMode(theChannel)==2?0:2);
  }
  AudioControl2.setChannelIndicatorLight(1,readOutChannel==3 ? (millis()&1024?B01:B00) : 0);
  AudioControl2.setButtonLight(2, AtemSwitcher.getAudioChannelMode(theChannel)==2);
  if (utils3.uniDirectionalSlider_hasMoved()  )  {
    Serial << "Pos3: " << utils3.uniDirectionalSlider_position() << "\n";
    AtemSwitcher.changeAudioChannelVolume(theChannel, calcAudioLevel(utils3.uniDirectionalSlider_position()))	;
  }
  ...
  AudioControl2.setButtonLight(3, AtemSwitcher.getAudioChannelMode(theChannel)==1);
  if(buttons & (1 << 3)) {
    AtemSwitcher.changeAudioChannelMode(theChannel, AtemSwitcher.getAudioChannelMode(theChannel)==2?0:2);
  }
  AudioControl2.setChannelIndicatorLight(2,readOutChannel==4 ? (millis()&1024?B01:B00) : 0);
  AudioControl2.setButtonLight(4, AtemSwitcher.getAudioChannelMode(theChannel)==2);
  if (utils4.uniDirectionalSlider_hasMoved()  )  {
    Serial << "Pos4: " << utils4.uniDirectionalSlider_position() << "\n";
    AtemSwitcher.changeAudioChannelVolume(theChannel, calcAudioLevel(utils4.uniDirectionalSlider_position()))	;
  }
  ...
  AudioControl3.setButtonLight(1, AtemSwitcher.getAudioChannelMode(theChannel)==1);
  if(buttons & (1 << 1)) {
    AtemSwitcher.changeAudioChannelMode(theChannel, AtemSwitcher.getAudioChannelMode(theChannel)==2?0:2);
  }
  AudioControl3.setChannelIndicatorLight(1,readOutChannel==5 ? (millis()&1024?B01:B00) : 0);
  AudioControl3.setButtonLight(2, AtemSwitcher.getAudioChannelMode(theChannel)==2);
  if (utils5.uniDirectionalSlider_hasMoved()  )  {
    Serial << "Pos5: " << utils5.uniDirectionalSlider_position() << "\n";
    AtemSwitcher.changeAudioChannelVolume(theChannel, calcAudioLevel(utils5.uniDirectionalSlider_position()))	;
  }
  ...
  AudioControl3.setButtonLight(3, AtemSwitcher.getAudioChannelMode(theChannel)==1);
  ...
  if (utils6.uniDirectionalSlider_hasMoved()  )  {
    ...
    AtemSwitcher.changeAudioChannelVolume(theChannel, calcAudioLevel(utils6.uniDirectionalSlider_position()))	;
  }
...
//    Serial << "Position 7 calc: " << calcAudioLevel(utils7.uniDirectionalSlider_position()) << "\n";
    AtemSwitcher.changeAudioMasterVolume(calcAudioLevel(utils7.uniDirectionalSlider_position()));
  }
  ...
  AudioControl4.VUmeter(AtemSwitcher.getAudioLevels(0),AtemSwitcher.getAudioLevels(1));
}


uint16_t calcAudioLevel(uint16_t sliderPosition)  {
  uint16_t tempsliderPosition = sliderPosition<10 ? 0 : (sliderPosition>990 ? 1000 : sliderPosition);
