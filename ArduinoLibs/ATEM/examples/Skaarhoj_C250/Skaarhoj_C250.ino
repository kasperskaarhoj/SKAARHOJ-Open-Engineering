...
void setup() { 
  ...
  Serial << F("\n- - - - - - - -\nSerial Started\n");  
    ...
    // Uses Analog pin A10 to detect from the LED voltage levels whether it is already on or not
  pinMode(24, INPUT);
  delay(3000);
  if (analogRead(A10)>700)  {  // 700 is the threshold value found by trying. If off, it's around 790, if on, it's around 650
    // Now, trying to turn the screen on:
    digitalWrite(24,0);  // First, set the next output of D4 to low
    pinMode(24, OUTPUT);  // Change D4 from an high impedance input to an output, effectively triggering a button press
    delay(500);  // Wait a while.
    pinMode(24, INPUT);  // Change D4 back to high impedance input.
  }
  ...
  utils.uniDirectionalSlider_init(10, 35, 35, A0);
...
uint16_t MVFieldMapping[] = {1,2,3,4, 5,6,7,8}; // Maps the multiviewer fields 1-4,5-8 in two rows to input source numbers. This needs to correspond to the switcher connected

void loop() {
  
  if (isConfigMode)  {
    webserver.processConnection();
    digitalWrite(23,(((unsigned long)millis()>>3) & B11000000) ? true : false);
  } 
  else {
    ...
    if (AtemSwitcher.hasInitialized())  {
      if (!AtemOnline)  {
        ...
        if (AtemSwitcher.getATEMmodel()==0)  {  // TVS:
          MVFieldMapping[3] = 3010;  // Media Player 1
          MVFieldMapping[4] = 4;
          MVFieldMapping[5] = 5;
          MVFieldMapping[6] = 6;
          MVFieldMapping[7] = 3020;  // Media Player 2
        }
      }
      ...
      digitalWrite(23,directlyToProgram);
      digitalWrite(22,!directlyToProgram);
...
void manageTouch()  {
  ...
  if (tState !=0)  {  // Different from zero? Then a touch even has happened:
    ...
    uint16_t MVfieldPressed = touchArea(utils.touch_getEndedXCoord(),utils.touch_getEndedYCoord());
    ...
    if (tState==1)  {  // Single tap somewhere:
      for (int i=1; i<=8; i++) {
            if ((MVfieldPressed==i && AtemSwitcher.getPreviewInput()==MVFieldMapping[i-1]) || (MVfieldPressed==17)) { // If Preview or a source ON preview is touched, bring in on.
              AtemSwitcher.doCut();
            }
      }
      if ((MVfieldPressed>=1 && MVfieldPressed<=8) && (AtemSwitcher.getPreviewInput()!=MVFieldMapping[MVfieldPressed-1])) {  // If any other source is touched, bring it to preview or program depending on mode:
