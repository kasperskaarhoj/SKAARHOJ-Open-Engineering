...
static uint8_t buffer[SKAARHOJOLED64x256_LCDHEIGHT * SKAARHOJOLED64x256_LCDWIDTH / 8];
...
void SkaarhojOLED64x256::drawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  // x is which column
  if (color == WHITE) 
    buffer[x/8+ y*SKAARHOJOLED64x256_LCDWIDTH/8] |= B10000000 >> (x%8);  
  else
    buffer[x/8+ y*SKAARHOJOLED64x256_LCDWIDTH/8] &= ~(B10000000 >> (x%8)); 
}
...
// clear everything
void SkaarhojOLED64x256::clearDisplay(void) {
  memset(buffer, 0, (SKAARHOJOLED64x256_LCDWIDTH*SKAARHOJOLED64x256_LCDHEIGHT/8));
}
...
void SkaarhojOLED64x256::display(uint8_t cs) {
  ...
  sendCommand(0x5C, cs);  // Enable write to data ram
    ...
    for (uint16_t i=0; i<(SKAARHOJOLED64x256_LCDWIDTH*SKAARHOJOLED64x256_LCDHEIGHT/8); i++) {
      ...
      fastSPIwrite((buffer[i]&B00100000 ? 0xF : 0x0) | (buffer[i]&B00010000 ? 0xF : 0x0) << 4);	// Third, Forth
      fastSPIwrite((buffer[i]&B10000000 ? 0xF : 0x0) | (buffer[i]&B01000000 ? 0xF : 0x0) << 4);	// First, Second

      fastSPIwrite((buffer[i]&B0010 ? 0xF : 0x0) | (buffer[i]&B0001 ? 0xF : 0x0) << 4);	// Third, Forth
      fastSPIwrite((buffer[i]&B1000 ? 0xF : 0x0) | (buffer[i]&B0100 ? 0xF : 0x0) << 4);	// First, Second
    }
