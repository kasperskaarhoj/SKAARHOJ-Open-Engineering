/*****************
 * This example will run the LED test sequence on 4 SkaarhojSIL boards on the I2C bus (if they exist)
 ...
 */
void buttonPressSession(uint8_t bDown, SkaarhojSIL &board)  {
  if (bDown)  {
    ...
    while(time+5000 > (unsigned long)millis()) {  
      ...
      if (bDown)  {
        ...
        Serial << bDown << "\n";
        for(uint8_t i=0; i<10;i++)  {  // Traverse all buttons
