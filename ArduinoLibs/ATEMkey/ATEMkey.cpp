...
void ATEMkey::delay(const unsigned int delayTimeMillis)	{	// Responsible delay function which keeps the ATEM run loop up! DO NOT USE INSIDE THIS CLASS! Recursion could happen...
...
uint16_t ATEMkey::getProgramInput() {
	return getProgramInputVideoSource(0);
}
uint16_t ATEMkey::getPreviewInput() {
	return getPreviewInputVideoSource(0);
}
boolean ATEMkey::getProgramTally(uint8_t inputNumber) {
	return (getTallyByIndexTallyFlags(inputNumber-1) & 1) >0 ? true : false;
}
boolean ATEMkey::getPreviewTally(uint8_t inputNumber) {
	return (getTallyByIndexTallyFlags(inputNumber-1) & 2) >0 ? true : false;
}
boolean ATEMkey::getUpstreamKeyerStatus(uint8_t inputNumber) {
	return getKeyerOnAirEnabled(0,inputNumber-1);
}
boolean ATEMkey::getUpstreamKeyerOnNextTransitionStatus(uint8_t inputNumber) {	// input 0 = background
	return (getTransitionNextTransition(0) & (0x01 << inputNumber)) ? true : false;
}
boolean ATEMkey::getDownstreamKeyerStatus(uint8_t inputNumber) {
	return getDownstreamKeyerOnAir(inputNumber-1);
}
uint16_t ATEMkey::getTransitionPosition() {
	return getTransitionPosition(0);
}
bool ATEMkey::getTransitionPreview()	{
	return getTransitionPreviewEnabled(0);
}
uint8_t ATEMkey::getTransitionType()	{
	return getTransitionStyle(0);
}
uint8_t ATEMkey::getTransitionMixTime() {
	return getTransitionMixRate(0);		// Transition time for Mix Transitions
}
boolean ATEMkey::getFadeToBlackState() {
	return getFadeToBlackStateFullyBlack(0);    // Active state of Fade-to-black
}
uint8_t ATEMkey::getFadeToBlackFrameCount() {
	return getFadeToBlackStateFramesRemaining(0);    // Returns current frame in the FTB
}
uint8_t ATEMkey::getFadeToBlackTime() {
	return getFadeToBlackRate(0);		// Transition time for Fade-to-black
}
bool ATEMkey::getDownstreamKeyTie(uint8_t keyer)	{
	return getDownstreamKeyerTie(keyer-1);
}
uint16_t ATEMkey::getAuxState(uint8_t auxOutput)  {
	return getAuxSourceInput(auxOutput-1);
}	
uint8_t ATEMkey::getMediaPlayerType(uint8_t mediaPlayer)  {
	return getMediaPlayerSourceType(mediaPlayer-1);
}
uint8_t ATEMkey::getMediaPlayerStill(uint8_t mediaPlayer)  {
	return getMediaPlayerSourceStillIndex(mediaPlayer-1)+1;
}
uint8_t ATEMkey::getMediaPlayerClip(uint8_t mediaPlayer)  {
	return getMediaPlayerSourceClipIndex(mediaPlayer-1)+1;
}
uint16_t ATEMkey::getAudioLevels(uint8_t channel)	{
	if (channel>0)	{
...
uint8_t ATEMkey::getAudioChannelMode(uint16_t channelNumber)	{
...
void ATEMkey::changeProgramInput(uint16_t inputNumber)  {
	setProgramInputVideoSource(0, inputNumber);
}
void ATEMkey::changePreviewInput(uint16_t inputNumber)  {
...
void ATEMkey::doAuto(uint8_t me)	{
	performAutoME(me);
}
void ATEMkey::fadeToBlackActivate()	{
	performFadeToBlackME(0);
}
void ATEMkey::changeTransitionPosition(word value)	{
	setTransitionPosition(0,value);
}
void ATEMkey::changeTransitionPositionDone()	{	// When the last value of the transition is sent (1000), send this one too (we are done, change tally lights and preview bus!)
	setTransitionPosition(0,0);
}
void ATEMkey::changeTransitionPreview(bool state)	{
	setTransitionPreviewEnabled(0, state);
}
void ATEMkey::changeTransitionType(uint8_t type)	{
	setTransitionStyle(0, type);
}
void ATEMkey::changeTransitionMixTime(uint8_t frames)	{
	setTransitionMixRate(0, frames);
}
void ATEMkey::changeFadeToBlackTime(uint8_t frames)	{
	setFadeToBlackRate(0, frames);
}
void ATEMkey::changeUpstreamKeyOn(uint8_t keyer, bool state)	{
	setKeyerOnAirEnabled(0, keyer-1, state);
}
void ATEMkey::changeUpstreamKeyNextTransition(uint8_t keyer, bool state)	{	// Supporting "Background" by "0"
...
void ATEMkey::changeDownstreamKeyOn(uint8_t keyer, bool state)	{
	setDownstreamKeyerOnAir(keyer-1, state);
}
void ATEMkey::changeDownstreamKeyTie(uint8_t keyer, bool state)	{
	setDownstreamKeyerTie(keyer-1, state);
}
void ATEMkey::doAutoDownstreamKeyer(uint8_t keyer)	{
	performDownstreamKeyerAutoKeyer(keyer-1);
}
void ATEMkey::changeAuxState(uint8_t auxOutput, uint16_t inputNumber)  {
	setAuxSourceInput(auxOutput-1, inputNumber);
}
void ATEMkey::settingsMemorySave()	{
	_prepareCommandPacket(PSTR("SRsv"),4);
	_finishCommandPacket();
}
void ATEMkey::settingsMemoryClear()	{
	_prepareCommandPacket(PSTR("SRcl"),4);
	_finishCommandPacket();
}
void ATEMkey::changeColorValue(uint8_t colorGenerator, uint16_t hue, uint16_t saturation, uint16_t lightness)  {
	commandBundleStart();
...
void ATEMkey::mediaPlayerSelectSource(uint8_t mediaPlayer, boolean movieclip, uint8_t sourceIndex)  {
	if (movieclip)	{
		commandBundleStart();
...
void ATEMkey::mediaPlayerClipStart(uint8_t mediaPlayer)  {
	setClipPlayerPlaying(mediaPlayer-1,true);
}

void ATEMkey::changeSwitcherVideoFormat(uint8_t format)	{
	setVideoModeFormat(format);
}



void ATEMkey::changeDVESettingsTemp(unsigned long Xpos,unsigned long Ypos,unsigned long Xsize,unsigned long Ysize)	{	// TEMP
	commandBundleStart();
...
void ATEMkey::changeDVEMaskTemp(unsigned long top,unsigned long bottom,unsigned long left,unsigned long right)	{	// TEMP
	// N/A
}
void ATEMkey::changeDVEBorder(bool enableBorder)	{	// TEMP
	setKeyDVEBorderEnabled(0, 0, enableBorder);
}

void ATEMkey::changeDVESettingsTemp_Rate(uint8_t rateFrames)	{	// TEMP
	setKeyDVERate(0,0,rateFrames);
}
void ATEMkey::changeDVESettingsTemp_RunKeyFrame(uint8_t runType)	{	// runType: 1=A, 2=B, 3=Full, 4=on of the others (with an extra paramter:)
	setRunFlyingKeyRuntoInfiniteindex(0,0, runType);	
}
void ATEMkey::changeKeyerMask(uint16_t topMask, uint16_t bottomMask, uint16_t leftMask, uint16_t rightMask) {
	changeKeyerMask(0, topMask, bottomMask, leftMask, rightMask);
}
void ATEMkey::changeKeyerMask(uint8_t keyer, uint16_t topMask, uint16_t bottomMask, uint16_t leftMask, uint16_t rightMask)	{
	commandBundleStart();
...
void ATEMkey::changeDownstreamKeyMask(uint8_t keyer, uint16_t topMask, uint16_t bottomMask, uint16_t leftMask, uint16_t rightMask)	{
		if (keyer>=1 && keyer<=2)	{
			commandBundleStart();
...
void ATEMkey::changeUpstreamKeyFillSource(uint8_t keyer, uint16_t inputNumber)	{
...
void ATEMkey::changeUpstreamKeyBlending(uint8_t keyer, bool preMultipliedAlpha, uint16_t clip, uint16_t gain, bool invKey)	{
	if (keyer>=1 && keyer<=4)	{	// Todo: Should match available keyers depending on model?
		commandBundleStart();
...
// TODO: ONLY clip works right now! there is a bug...
void ATEMkey::changeDownstreamKeyBlending(uint8_t keyer, bool preMultipliedAlpha, uint16_t clip, uint16_t gain, bool invKey)	{
...
void ATEMkey::changeDownstreamKeyFillSource(uint8_t keyer, uint16_t inputNumber)	{
	if (keyer>=1 && keyer<=2)	{	// Todo: Should match available keyers depending on model?
	  	setDownstreamKeyerFillSource(keyer, inputNumber);	
	}
}

void ATEMkey::changeDownstreamKeyKeySource(uint8_t keyer, uint16_t inputNumber)	{
	if (keyer>=1 && keyer<=2)	{	// Todo: Should match available keyers depending on model?
...
void ATEMkey::changeAudioChannelMode(uint16_t channelNumber, uint8_t mode)	{	// Mode: 0=Off, 1=On, 2=AFV
	setAudioMixerInputMixOption(channelNumber, mode);
}
void ATEMkey::changeAudioChannelVolume(uint16_t channelNumber, uint16_t volume)	{
...
void ATEMkey::changeAudioMasterVolume(uint16_t volume)	{
	setAudioMixerMasterVolume(volume);
}
void ATEMkey::sendAudioLevelNumbers(bool enable)	{
...
void ATEMkey::setAudioLevelReadoutChannel(uint16_t AMLv)	{
	_ATEM_AMLv_channel = AMLv;	// Should check that it's in range 0-12
}



void ATEMkey::setWipeReverseDirection(bool reverse) {
 ...
 */
long ATEMkey::getAudioMixerLevelsMasterLeft() {
 ...
 */
long ATEMkey::getAudioMixerLevelsMasterRight() {
 ...
 */
long ATEMkey::getAudioMixerLevelsMonitor() {
 ...
 */
long ATEMkey::getAudioMixerLevelsSourceLeft() {
	return atemAudioMixerLevelsSourceLeft;
}
 ...
 */
long ATEMkey::getAudioMixerLevelsSourceRight() {
	return atemAudioMixerLevelsSourceRight;
}	
...
void ATEMkey::setKeyChromaHue(uint8_t mE, uint8_t keyer, uint16_t hue) {
	 ...
	 * gain 	0-1000: 0-100%
	 */
	void ATEMkey::setKeyChromaGain(uint8_t mE, uint8_t keyer, uint16_t gain) {
	 ...
	 */
	void ATEMkey::setKeyChromaYSuppress(uint8_t mE, uint8_t keyer, uint16_t ySuppress) {
	 ...
	 * lift 	0-1000: 0-100%
	 */
	void ATEMkey::setKeyChromaLift(uint8_t mE, uint8_t keyer, uint16_t lift) {
	 ...
	 * narrow 	Bit 0: On/Off
	 */
	void ATEMkey::setKeyChromaNarrow(uint8_t mE, uint8_t keyer, bool narrow) {
...
void ATEMkey::setKeyerType(uint8_t mE, uint8_t keyer, uint8_t type) {
...
void ATEMkey::_parseGetCommands(const char *cmdStr)	{
	uint8_t mE,keyer,colorGenerator,aUXChannel,mediaPlayer;
	uint16_t audioSource,sources;
	long temp;
	 ...
	 */
	uint16_t ATEMkey::getProtocolVersionMajor() {
	 ...
	 */
	uint16_t ATEMkey::getProtocolVersionMinor() {
	 ...
	 */
	uint8_t ATEMkey::getVideoModeFormat() {
	 ...
	 */
	void ATEMkey::setVideoModeFormat(uint8_t format) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint16_t ATEMkey::getProgramInputVideoSource(uint8_t mE) {
	 ...
	 */
	void ATEMkey::setProgramInputVideoSource(uint8_t mE, uint16_t videoSource) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint16_t ATEMkey::getPreviewInputVideoSource(uint8_t mE) {
	 ...
	 * videoSource 	(See video source list)
	 */
	void ATEMkey::setPreviewInputVideoSource(uint8_t mE, uint16_t videoSource) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	void ATEMkey::performCutME(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	void ATEMkey::performAutoME(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint8_t ATEMkey::getTransitionStyle(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint8_t ATEMkey::getTransitionNextTransition(uint8_t mE) {
	 ...
	 */
	void ATEMkey::setTransitionStyle(uint8_t mE, uint8_t style) {
	 ...
	 */
	void ATEMkey::setTransitionNextTransition(uint8_t mE, uint8_t nextTransition) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	bool ATEMkey::getTransitionPreviewEnabled(uint8_t mE) {
	 ...
	 */
	void ATEMkey::setTransitionPreviewEnabled(uint8_t mE, bool enabled) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	bool ATEMkey::getTransitionInTransition(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint8_t ATEMkey::getTransitionFramesRemaining(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint16_t ATEMkey::getTransitionPosition(uint8_t mE) {
	 ...
	 * position 	0-9999
	 */
	void ATEMkey::setTransitionPosition(uint8_t mE, uint16_t position) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint8_t ATEMkey::getTransitionMixRate(uint8_t mE) {
	 ...
	 * rate 	1-250: Frames
	 */
	void ATEMkey::setTransitionMixRate(uint8_t mE, uint8_t rate) {
	 ...
	 * rate 	1-250: Frames
	 */
	void ATEMkey::setTransitionWipeRate(uint8_t mE, uint8_t rate) {
	 ...
	 */
	void ATEMkey::setTransitionWipePattern(uint8_t mE, uint8_t pattern) {
	 ...
	 * width 	0-10000: 0-100%
	 */
	void ATEMkey::setTransitionWipeWidth(uint8_t mE, uint16_t width) {
	 ...
	 */
	void ATEMkey::setTransitionWipeFillSource(uint8_t mE, uint16_t fillSource) {
	 ...
	 */
	void ATEMkey::setTransitionWipeSymmetry(uint8_t mE, uint16_t symmetry) {
	 ...
	 */
	void ATEMkey::setTransitionWipeSoftness(uint8_t mE, uint16_t softness) {
	 ...
	 */
	void ATEMkey::setTransitionWipePositionX(uint8_t mE, uint16_t positionX) {
	 ...
	 */
	void ATEMkey::setTransitionWipePositionY(uint8_t mE, uint16_t positionY) {
	 ...
	 */
	void ATEMkey::setTransitionWipeReverse(uint8_t mE, bool reverse) {
	 ...
	 */
	void ATEMkey::setTransitionWipeFlipFlop(uint8_t mE, bool flipFlop) {
	 ...
	 * keyer 	0-3: Keyer 1-4
	 */
	bool ATEMkey::getKeyerOnAirEnabled(uint8_t mE, uint8_t keyer) {
	 ...
	 * enabled 	Bit 0: On/Off
	 */
	void ATEMkey::setKeyerOnAirEnabled(uint8_t mE, uint8_t keyer, bool enabled) {
	 ...
	 * masked 	Bit 0: On/Off
	 */
	void ATEMkey::setKeyerMasked(uint8_t mE, uint8_t keyer, bool masked) {
	 ...
	 */
	void ATEMkey::setKeyerTop(uint8_t mE, uint8_t keyer, int top) {
	 ...
	 */
	void ATEMkey::setKeyerBottom(uint8_t mE, uint8_t keyer, int bottom) {
	 ...
	 */
	void ATEMkey::setKeyerLeft(uint8_t mE, uint8_t keyer, int left) {
	 ...
	 */
	void ATEMkey::setKeyerRight(uint8_t mE, uint8_t keyer, int right) {
	 ...
	 * fillSource 	(See video source list)
	 */
	void ATEMkey::setKeyerFillSource(uint8_t mE, uint8_t keyer, uint16_t fillSource) {
	 ...
	 */
	void ATEMkey::setKeyLumaPreMultiplied(uint8_t mE, uint8_t keyer, bool preMultiplied) {
	 ...
	 * clip 	0-1000: 0-100%
	 */
	void ATEMkey::setKeyLumaClip(uint8_t mE, uint8_t keyer, uint16_t clip) {
	 ...
	 * gain 	0-1000: 0-100%
	 */
	void ATEMkey::setKeyLumaGain(uint8_t mE, uint8_t keyer, uint16_t gain) {
	 ...
	 */
	void ATEMkey::setKeyLumaInvertKey(uint8_t mE, uint8_t keyer, bool invertKey) {
	 ...
	 */
	void ATEMkey::setKeyDVESizeX(uint8_t mE, uint8_t keyer, long sizeX) {
	 ...
	 */
	void ATEMkey::setKeyDVESizeY(uint8_t mE, uint8_t keyer, long sizeY) {
	 ...
	 */
	void ATEMkey::setKeyDVEPositionX(uint8_t mE, uint8_t keyer, long positionX) {
	 ...
	 */
	void ATEMkey::setKeyDVEPositionY(uint8_t mE, uint8_t keyer, long positionY) {
	 ...
	 */
	void ATEMkey::setKeyDVERotation(uint8_t mE, uint8_t keyer, long rotation) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderEnabled(uint8_t mE, uint8_t keyer, bool borderEnabled) {
	 ...
	 * shadow 	Bit 0: On/Off
	 */
	void ATEMkey::setKeyDVEShadow(uint8_t mE, uint8_t keyer, bool shadow) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderBevel(uint8_t mE, uint8_t keyer, uint8_t borderBevel) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderOuterWidth(uint8_t mE, uint8_t keyer, uint16_t borderOuterWidth) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderInnerWidth(uint8_t mE, uint8_t keyer, uint16_t borderInnerWidth) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderOuterSoftness(uint8_t mE, uint8_t keyer, uint8_t borderOuterSoftness) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderInnerSoftness(uint8_t mE, uint8_t keyer, uint8_t borderInnerSoftness) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderBevelSoftness(uint8_t mE, uint8_t keyer, uint8_t borderBevelSoftness) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderBevelPosition(uint8_t mE, uint8_t keyer, uint8_t borderBevelPosition) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderOpacity(uint8_t mE, uint8_t keyer, uint8_t borderOpacity) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderHue(uint8_t mE, uint8_t keyer, uint16_t borderHue) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderSaturation(uint8_t mE, uint8_t keyer, uint16_t borderSaturation) {
	 ...
	 */
	void ATEMkey::setKeyDVEBorderLuma(uint8_t mE, uint8_t keyer, uint16_t borderLuma) {
	 ...
	 */
	void ATEMkey::setKeyDVELightSourceDirection(uint8_t mE, uint8_t keyer, uint16_t lightSourceDirection) {
	 ...
	 */
	void ATEMkey::setKeyDVELightSourceAltitude(uint8_t mE, uint8_t keyer, uint8_t lightSourceAltitude) {
	 ...
	 * rate 	1-250: Frames
	 */
	void ATEMkey::setKeyDVERate(uint8_t mE, uint8_t keyer, uint8_t rate) {
	 ...
	 */
	void ATEMkey::setRunFlyingKeyKeyFrame(uint8_t mE, uint8_t keyer, uint8_t keyFrame) {
	 ...
	 * runtoInfiniteindex 	
	 */
	void ATEMkey::setRunFlyingKeyRuntoInfiniteindex(uint8_t mE, uint8_t keyer, uint8_t runtoInfiniteindex) {
	 ...
	 * fillSource 	(See video source list)
	 */
	void ATEMkey::setDownstreamKeyerFillSource(uint8_t keyer, uint16_t fillSource) {
	 ...
	 */
	void ATEMkey::setDownstreamKeyerKeySource(uint8_t keyer, uint16_t keySource) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerTie(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	uint8_t ATEMkey::getDownstreamKeyerRate(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerPreMultiplied(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	uint16_t ATEMkey::getDownstreamKeyerClip(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	uint16_t ATEMkey::getDownstreamKeyerGain(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerInvertKey(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerMasked(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	int ATEMkey::getDownstreamKeyerTop(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	int ATEMkey::getDownstreamKeyerBottom(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	int ATEMkey::getDownstreamKeyerLeft(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	int ATEMkey::getDownstreamKeyerRight(uint8_t keyer) {
	 ...
	 * tie 	Bit 0: On/Off
	 */
	void ATEMkey::setDownstreamKeyerTie(uint8_t keyer, bool tie) {
	 ...
	 * preMultiplied 	Bit 0: On/Off
	 */
	void ATEMkey::setDownstreamKeyerPreMultiplied(uint8_t keyer, bool preMultiplied) {
	 ...
	 * clip 	0-1000: 0-100%
	 */
	void ATEMkey::setDownstreamKeyerClip(uint8_t keyer, uint16_t clip) {
	 ...
	 * gain 	0-1000: 0-100%
	 */
	void ATEMkey::setDownstreamKeyerGain(uint8_t keyer, uint16_t gain) {
	 ...
	 * invertKey 	Bit 0: On/Off
	 */
	void ATEMkey::setDownstreamKeyerInvertKey(uint8_t keyer, bool invertKey) {
	 ...
	 * masked 	Bit 0: On/Off
	 */
	void ATEMkey::setDownstreamKeyerMasked(uint8_t keyer, bool masked) {
	 ...
	 * top 	-9000-9000: -9.00-9.00
	 */
	void ATEMkey::setDownstreamKeyerTop(uint8_t keyer, int top) {
	 ...
	 * bottom 	-9000-9000: -9.00-9.00
	 */
	void ATEMkey::setDownstreamKeyerBottom(uint8_t keyer, int bottom) {
	 ...
	 * left 	-16000-16000: -9.00-9.00
	 */
	void ATEMkey::setDownstreamKeyerLeft(uint8_t keyer, int left) {
	 ...
	 * right 	-16000-16000: -9.00-9.00
	 */
	void ATEMkey::setDownstreamKeyerRight(uint8_t keyer, int right) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	void ATEMkey::performDownstreamKeyerAutoKeyer(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerOnAir(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerInTransition(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	bool ATEMkey::getDownstreamKeyerIsAutoTransitioning(uint8_t keyer) {
	 ...
	 * keyer 	0: DSK1, 1: DSK2
	 */
	uint8_t ATEMkey::getDownstreamKeyerFramesRemaining(uint8_t keyer) {
	 ...
	 * onAir 	Bit 0: On/Off
	 */
	void ATEMkey::setDownstreamKeyerOnAir(uint8_t keyer, bool onAir) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint8_t ATEMkey::getFadeToBlackRate(uint8_t mE) {
	 ...
	 * rate 	1-250: Frames
	 */
	void ATEMkey::setFadeToBlackRate(uint8_t mE, uint8_t rate) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	bool ATEMkey::getFadeToBlackStateFullyBlack(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	bool ATEMkey::getFadeToBlackStateInTransition(uint8_t mE) {
	 ...
	 * mE 	0: ME1, 1: ME2
	 */
	uint8_t ATEMkey::getFadeToBlackStateFramesRemaining(uint8_t mE) {
		 ...
		 * mE 	0: ME1, 1: ME2
		 */
		void ATEMkey::performFadeToBlackME(uint8_t mE) {
	 ...
	 */
	void ATEMkey::setColorGeneratorHue(uint8_t colorGenerator, uint16_t hue) {
	 ...
	 */
	void ATEMkey::setColorGeneratorSaturation(uint8_t colorGenerator, uint16_t saturation) {
	 ...
	 * luma 	0-1000: 0-100.0%
	 */
	void ATEMkey::setColorGeneratorLuma(uint8_t colorGenerator, uint16_t luma) {
	 ...
	 */
	uint16_t ATEMkey::getAuxSourceInput(uint8_t aUXChannel) {
	 ...
	 */
	void ATEMkey::setAuxSourceInput(uint8_t aUXChannel, uint16_t input) {
	 ...
	 */
	void ATEMkey::setClipPlayerPlaying(uint8_t mediaPlayer, bool playing) {
	 ...
	 * loop 	Bit 0: On/Off
	 */
	void ATEMkey::setClipPlayerLoop(uint8_t mediaPlayer, bool loop) {
	 ...
	 */
	void ATEMkey::setClipPlayerAtBeginning(uint8_t mediaPlayer, bool atBeginning) {
	 ...
	 * clipFrame 	
	 */
	void ATEMkey::setClipPlayerClipFrame(uint8_t mediaPlayer, uint16_t clipFrame) {
	 ...
	 * mediaPlayer 	0: Media Player 1, 1: Media Player 2
	 */
	uint8_t ATEMkey::getMediaPlayerSourceType(uint8_t mediaPlayer) {
	 ...
	 * mediaPlayer 	0: Media Player 1, 1: Media Player 2
	 */
	uint8_t ATEMkey::getMediaPlayerSourceStillIndex(uint8_t mediaPlayer) {
	 ...
	 * mediaPlayer 	0: Media Player 1, 1: Media Player 2
	 */
	uint8_t ATEMkey::getMediaPlayerSourceClipIndex(uint8_t mediaPlayer) {
	 ...
	 * type 	1: Still, 2: Clip
	 */
	void ATEMkey::setMediaPlayerSourceType(uint8_t mediaPlayer, uint8_t type) {
	 ...
	 */
	void ATEMkey::setMediaPlayerSourceStillIndex(uint8_t mediaPlayer, uint8_t stillIndex) {
	 ...
	 */
	void ATEMkey::setMediaPlayerSourceClipIndex(uint8_t mediaPlayer, uint8_t clipIndex) {
	 ...
	 */
	uint8_t ATEMkey::getAudioMixerInputMixOption(uint16_t audioSource) {
	 ...
	 * audioSource 	(See audio source list)
	 */
	uint16_t ATEMkey::getAudioMixerInputVolume(uint16_t audioSource) {
	 ...
	 * audioSource 	(See audio source list)
	 */
	int ATEMkey::getAudioMixerInputBalance(uint16_t audioSource) {
	 ...
	 */
	void ATEMkey::setAudioMixerInputMixOption(uint16_t audioSource, uint8_t mixOption) {
	 ...
	 * volume 	0-65381: (DB)
	 */
	void ATEMkey::setAudioMixerInputVolume(uint16_t audioSource, uint16_t volume) {
	 ...
	 */
	void ATEMkey::setAudioMixerInputBalance(uint16_t audioSource, int balance) {
	 ...
	 * volume 	0-65381: (DB)
	 */
	void ATEMkey::setAudioMixerMasterVolume(uint16_t volume) {
	 ...
	 * enable 	Bit 0: On/Off
	 */
	void ATEMkey::setAudioLevelsEnable(bool enable) {
	 ...
	 */
	uint16_t ATEMkey::getTallyByIndexSources() {
	 ...
	 */
	uint8_t ATEMkey::getTallyByIndexTallyFlags(uint16_t sources) {
