		...
		switch(_section)	{
			case 2: 	// VIDEOHUB DEVICE
				if (isNextPartOfBuffer_P(PSTR("Device present: ")))	{
					...
					if (_serialOutput>1) Serial.print(F("_devicePresent="));
					if (_serialOutput>1) Serial.println(_devicePresent);
				}
				if (isNextPartOfBuffer_P(PSTR("Video inputs: ")))	{
					_numInputs = parseInt();
					if (_serialOutput>1) Serial.print(F("_numInputs="));
					if (_serialOutput>1) Serial.println(_numInputs);
				}
				if (isNextPartOfBuffer_P(PSTR("Video processing units: ")))	{
					_numProcUnits = parseInt();
					if (_serialOutput>1) Serial.print(F("_numProcUnits="));
					if (_serialOutput>1) Serial.println(_numProcUnits);
				}
				if (isNextPartOfBuffer_P(PSTR("Video outputs: ")))	{
					_numOutputs = parseInt();
					if (_serialOutput>1) Serial.print(F("_numOutputs="));
					if (_serialOutput>1) Serial.println(_numOutputs);
				}
				if (isNextPartOfBuffer_P(PSTR("Video monitoring outputs: ")))	{
					_numMonOutputs = parseInt();
					if (_serialOutput>1) Serial.print(F("_numMonOutputs="));
					if (_serialOutput>1) Serial.println(_numMonOutputs);
				}
				if (isNextPartOfBuffer_P(PSTR("Serial ports: ")))	{
					_serialPorts = parseInt();
					if (_serialOutput>1) Serial.print(F("_serialPorts="));
					if (_serialOutput>1) Serial.println(_serialPorts);
				}
			break;
			case 3: 	// INPUT LABELS
				inputNum = parseInt();
				if (inputNum < ClientBMDVideohubTCP_NUMINPUTS)	{
					...
					if (_serialOutput>1) Serial.print(F("_inputLabels "));
					if (_serialOutput>1) Serial.print(inputNum);
					if (_serialOutput>1) Serial.print(F("="));
					if (_serialOutput>1) Serial.println(_inputLabels[inputNum]);
				}
			break;
			case 4: 	// OUTPUT LABELS
				...
				if (outputNum < ClientBMDVideohubTCP_NUMOUTPUTS)	{
					...
					if (_serialOutput>1) Serial.print(F("_outputLabels "));
					if (_serialOutput>1) Serial.print(outputNum);
					if (_serialOutput>1) Serial.print(F("="));
					if (_serialOutput>1) Serial.println(_outputLabels[outputNum]);
				}
			break;
			case 5: 	// VIDEO OUTPUT LOCKS
				...
				if (outputNum < ClientBMDVideohubTCP_NUMOUTPUTS)	{
					...
					if (_serialOutput>1) Serial.print(F("_outputLock "));
					if (_serialOutput>1) Serial.print(outputNum);
					if (_serialOutput>1) Serial.print(F("="));
					if (_serialOutput>1) Serial.println(_outputLocks[outputNum]);
				}
			break;
			case 6: 	// VIDEO OUTPUT ROUTING
				...
				if (outputNum < ClientBMDVideohubTCP_NUMOUTPUTS)	{
					...
					if (_serialOutput>1) Serial.print(outputNum);
					if (_serialOutput>1) Serial.print(F("="));
					if (_serialOutput>1) Serial.println(_outputRouting[outputNum]);
				}
			break;
		}
	}
