...
static uint8_t buffer[SKAARHOJSMARTSWITCH_LCDWIDTH * SKAARHOJSMARTSWITCH_LCDHEIGHT / 8];
...
// clear everything
void SkaarhojSmartSwitch2::clearDisplay(void) {
...
void SkaarhojSmartSwitch2::display(uint8_t cs) {
	...
	for (uint16_t i=0; i<(SKAARHOJSMARTSWITCH_LCDWIDTH * SKAARHOJSMARTSWITCH_LCDHEIGHT/8); i++) {
		shiftOut(_dataPin, _clockPin, MSBFIRST, buffer[i]);
    }
