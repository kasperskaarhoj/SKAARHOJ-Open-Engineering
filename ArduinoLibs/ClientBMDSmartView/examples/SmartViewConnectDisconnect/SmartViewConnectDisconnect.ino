/*****************
 * Example: Smart View connect/disconnect test
 * Connects, sets border, disconnects, round robin with multiple SmartView monitors
 *
 * - kasper
 */
/*****************
...
void loop() {
  ...
  for(uint8_t a=0; a<numberOfSmartViews; a++)  {
    ...
    SmartView.begin(IPAddress(192, 168, 10, lastDigit));	 // <= SETUP (the IP address of the SmartView)
    SmartView.serialOutput(1);
    SmartView.connect();
    SmartView.waitForReady(10000);
    if (SmartView.isConnected())  {
      SmartView.setBorder(mon%2,(border%3)+1);
      SmartView.waitForReady(10000);
